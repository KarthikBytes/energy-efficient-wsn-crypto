...
// NS-3.42 MEMOSTP Simulation - Fixed version with NetAnim
...
private:
    void SendPacket() {
        ...
        if (g_anim && destNode < 25) {  // Limit visualization to reasonable node IDs
            //g_anim->UpdateNodeColor(m_nodeId, 255, 255, 0); // Yellow for TX
            //Simulator::Schedule(Seconds(0.2), &AnimationInterface::UpdateNodeColor, 
            //                   g_anim, m_nodeId, 255, 255, 255);
        }
    ...
    void HandleRead(Ptr<Socket> socket) {
        ...
        while ((packet = socket->RecvFrom(from))) {
            ...
            if (g_anim) {
                //g_anim->UpdateNodeColor(m_nodeId, 255, 165, 0); // Orange for RX
                //Simulator::Schedule(Seconds(0.2), &AnimationInterface::UpdateNodeColor, 
                //                   g_anim, m_nodeId, 255, 255, 255);
            }
...
int main(int argc, char *argv[]) {
    ...
    bool enable_netanim = true;  // NEW: NetAnim control
    std::string animFile = "memostp-animation.xml";  // NEW: Animation file
    ...
    cmd.AddValue("enableNetAnim", "Enable NetAnim visualization", enable_netanim);  // NEW
    cmd.AddValue("animFile", "NetAnim output file", animFile);  // NEW
    cmd.Parse(argc, argv);
    ...
    std::cout << "└─ NetAnim: " << (enable_netanim ? "Enabled (" + animFile + ")" : "Disabled") << std::endl;  // UPDATED
