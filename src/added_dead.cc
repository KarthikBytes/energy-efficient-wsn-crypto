...
std::vector<uint32_t> g_deadNodes; // Track dead nodes
std::vector<double> g_nodeUsefulness; // Track node usefulness scores
std::vector<double> g_nodeEnergyLevels; // Track remaining energy per node
...
////////////////////////////////////////////////////////
// NODE DEATH AND USEFULNESS TRACKING                //
////////////////////////////////////////////////////////
void KillNode(uint32_t nodeId, double time) {
    if (std::find(g_deadNodes.begin(), g_deadNodes.end(), nodeId) == g_deadNodes.end()) {
        g_deadNodes.push_back(nodeId);
        
        // Emit node death event
        std::cout << "{"
                  << "\"time\":" << std::fixed << std::setprecision(3) << time << ","
                  << "\"event\":\"node_death\","
                  << "\"nodeId\":" << nodeId
                  << "}" << std::endl;
        
        std::cout << "\033[1;31m☠️  Node " << nodeId << " died at time " << time << "s\033[0m" << std::endl;
    }
}

void UpdateNodeUsefulness(uint32_t nodeId, double usefulness) {
    if (nodeId < g_nodeUsefulness.size()) {
        g_nodeUsefulness[nodeId] = usefulness;
        
        // Emit usefulness update event
        std::cout << "{"
                  << "\"time\":" << std::fixed << std::setprecision(3) 
                  << ns3::Simulator::Now().GetSeconds() << ","
                  << "\"event\":\"usefulness_update\","
                  << "\"nodeId\":" << nodeId << ","
                  << "\"usefulness\":" << std::fixed << std::setprecision(4) << usefulness
                  << "}" << std::endl;
    }
}

void InitializeNodeTracking(uint32_t nNodes) {
    g_nodeUsefulness.resize(nNodes, 1.0); // Start with full usefulness
    g_nodeEnergyLevels.resize(nNodes, 100.0); // Start with 100% energy
}
...
public:
    CryptoTestApplication() : m_socket(0), m_peerPort(0), m_packetSize(512),
                             ...
                             m_energyConsumed(0.0), m_packetsForwarded(0) {}
...
private:
    void SendPacket() {
        ...
        m_energyConsumed += 0.0015; // 1.5 mJ per transmission
        if (m_nodeId < g_nodeEnergyLevels.size()) {
            g_nodeEnergyLevels[m_nodeId] -= 0.15; // Reduce energy by 0.15%
            
            // Check if node should die due to energy depletion
            if (g_nodeEnergyLevels[m_nodeId] <= 0 && 
                std::find(g_deadNodes.begin(), g_deadNodes.end(), m_nodeId) == g_deadNodes.end()) {
                KillNode(m_nodeId, Simulator::Now().GetSeconds());
            }
        }
    ...
    void HandleRead(Ptr<Socket> socket) {
        ...
        while ((packet = socket->RecvFrom(from))) {
            ...
            // UPDATE USEFULNESS
            m_packetsForwarded++;
            if (m_nodeId < g_nodeUsefulness.size()) {
                // Calculate usefulness based on packets forwarded and energy remaining
                double energyFactor = g_nodeEnergyLevels[m_nodeId] / 100.0;
                double usefulness = (m_packetsForwarded / 100.0) * energyFactor;
                UpdateNodeUsefulness(m_nodeId, usefulness);
            }
...
int main(int argc, char *argv[]) {
    ...
    // Remove EnergyDetectionThreshold - not valid in NS-3.42
    ...
    // Add random node deaths
    Ptr<UniformRandomVariable> rand = CreateObject<UniformRandomVariable>();
    for (uint32_t i = 0; i < nNodes / 10; i++) { // 10% of nodes may die
        double deathTime = rand->GetValue(5.0, simulationTime - 5.0);
        uint32_t nodeToDie = rand->GetInteger(0, nNodes - 1);
        
        Simulator::Schedule(Seconds(deathTime), &KillNode, nodeToDie, deathTime);
    }
    ...
    uint32_t aliveNodes = nNodes - g_deadNodes.size();
    double avgUsefulness = 0.0;
    double avgEnergyRemaining = 0.0;
    uint32_t usefulNodes = 0;
    
    for (uint32_t i = 0; i < nNodes; i++) {
        if (std::find(g_deadNodes.begin(), g_deadNodes.end(), i) == g_deadNodes.end()) {
            avgEnergyRemaining += g_nodeEnergyLevels[i];
            avgUsefulness += g_nodeUsefulness[i];
            if (g_nodeUsefulness[i] > 0.5) usefulNodes++;
        }
    }
    
    if (aliveNodes > 0) {
        avgEnergyRemaining /= aliveNodes;
        avgUsefulness /= aliveNodes;
    }
    ...
    EmitEvent("node_stats", aliveNodes, g_deadNodes.size(), usefulNodes);
    ...
    std::cout << "\n\033[1;33mNODE STATISTICS:\033[0m" << std::endl;
    std::cout << "├─ Total Nodes:        " << nNodes << std::endl;
    std::cout << "├─ Alive Nodes:        " << aliveNodes << " (" 
              << std::fixed << std::setprecision(1) 
              << (double)aliveNodes/nNodes*100 << "%)" << std::endl;
    std::cout << "├─ Dead Nodes:         " << g_deadNodes.size() << std::endl;
    std::cout << "├─ Useful Nodes (>0.5): " << usefulNodes << " (" 
              << std::fixed << std::setprecision(1) 
              << (double)usefulNodes/aliveNodes*100 << "%)" << std::endl;
    std::cout << "├─ Avg Usefulness:     " << std::fixed << std::setprecision(4) 
              << avgUsefulness << std::endl;
    std::cout << "└─ Avg Energy Remaining: " << std::fixed << std::setprecision(2) 
              << avgEnergyRemaining << "%" << std::endl;

    if (!g_deadNodes.empty()) {
        std::cout << "\n\033[1;31mDead Nodes: ";
        for (size_t i = 0; i < std::min<size_t>(10, g_deadNodes.size()); i++) {
            std::cout << g_deadNodes[i];
            if (i < g_deadNodes.size() - 1 && i < 9) std::cout << ", ";
        }
        if (g_deadNodes.size() > 10) std::cout << "...";
